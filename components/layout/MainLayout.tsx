...
  useEffect(() => {
    // CORRECTED: Use absolute URLs
    fetch(`${BASE_API_URL}/api/tasks`).then(res => res.json()).then(setTasks);
    fetch(`${BASE_API_URL}/api/announcements`).then(res => res.json()).then(setAnnouncements);
  }, []);
...
    );
    // CORRECTED: Use absolute URL
    await fetch(`${BASE_API_URL}/api/tasks/${taskId}/toggle`, { method: 'PUT' });
  };
