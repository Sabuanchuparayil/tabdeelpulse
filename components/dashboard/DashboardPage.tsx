...
    useEffect(() => {
        // CORRECTED: Use absolute URLs for all fetch calls
        fetch(`${BASE_API_URL}/api/finance/collections`).then(res => res.json()).then(setCollections);
        fetch(`${BASE_API_URL}/api/finance/instructions`).then(res => res.json()).then(setInstructions);
        fetch(`${BASE_API_URL}/api/service-jobs`).then(res => res.json()).then(setServiceJobs);
    }, []);
